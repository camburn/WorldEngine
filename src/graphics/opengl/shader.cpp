...
std::unordered_map<std::string, bool> log_once_map;

void log_invalid_uniform(std::string name) {
    if (log_once_map.count(name) == 1) {
        return;
    }
    std::cout << "RENDER:: Invalid uniform name: " << name << std::endl;
    log_once_map.emplace(name, true);
}
